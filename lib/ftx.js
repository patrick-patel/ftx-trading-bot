...
ws.on('update', msg => { // alter after updating below to all fills
  ...
  if (msg.data) {
    ...
    .then(candle => {
      ...
      if (side === 'buy') {
        var triggerPrice = candle[0].low - orderAdj;
      ...
      if (side === 'sell') {
        var triggerPrice = candle[0].high + orderAdj;
...
ws.subscribe({ // need to subscribe to all fills and evaluate obj
  channel: 'fills',
  market: 'LINK/BTC'
})
