...
ws.on('update', msg => {
  ...
  if (msg.data) {
    ...
    .then(candle => {
      ...
      if (side === 'buy') {
        var triggerPrice = candle[0].low - sellSideAdj;
        console.log('triggerPrice: ', triggerPrice);
        console.log('size variable in then: ', size);
        console.log('size from msg: ', msg.data.size);
        var size = msg.data.size;
      ...
      if (side === 'sell') {
        var triggerPrice = candle[0].high + buySideAdj;
        var size = msg.data.size / candle[0].high * 0.9;
        client.placeTriggerOrder({
          'market': 'LINK/BTC',
          'side': 'buy',
          'triggerPrice': triggerPrice,
