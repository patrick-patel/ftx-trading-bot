...
let establishRESTConnection = ({
  ...
  var connection = {
    client: new RestClient(api_key, secret, restClientOptions),
    freeBTCScaler: 1 / (6 * 2), // change to divide by numactivepairs
    orderAdj: 0.0025 * Math.random() + 0.0005
  };
  return connection;
}
...
let postStopMarketBuyOrder = (client, triggerPrice, size, currentPrice, pair, orderAdj, freeBTCScaler) => {
...
let postStopMarketSellOrder = (client, triggerPrice, size, pair, orderAdj, freeBTCScaler) => {
...
let establishWSConnection = (credential) => {
  ...
  ws.on('update', msg => {
    ...
    if (msg.data) {
      ...
      var hrs = Object.keys(credential.isSubscribedTo[pair]);
      console.log('hr keys array: ', hrs);
      var hr = hrs.find(key => credential.isSubscribedTo[pair][key] === true);
